...
- ✅ **Task 11 (Merge suggestions)** - DEVIATION from plan: `mergeSuggestion`/`closeSuggestion` already existed (status-only) in `src/lib/actions/tasteTests.ts`, so they were upgraded IN PLACE there (not in versions.ts). Merge now: applies each `{ ingredient, from, to }` diff entry to the matching ComponentIngredient (parsed via new `src/lib/parse-amount.ts` + 10 tests), records a version credited to the SUGGESTION author, increments tweakCount, marks MERGED, notifies. Suggestion zod schema field renamed `field`→`ingredient` to match UI/DB shape. SuggestionCard buttons wired (owner-only, confirm dialog). Seeded 6 OPEN suggestions with real mergeable diffs (in seed.ts AND inserted directly into the live DB via one-off script - re-seeding also produces them).
- ✅ **Task 12 (Blame)** - `src/lib/blame.ts` (`computeBlame`, 5 tests), `src/components/recipe/BlameView.tsx` (gutter = avatar + tweak message + age, grouped by component, ingredients + steps sections). Recipe page computes blame server-side from ALL snapshotted versions (asc) and passes `blame` prop; instructions SectionFile gets an Eye/GitCommit toggle (only when blame exists). Smoke-tested: recipe page 200, compare page renders real fork diffs.
- ✅ **Task 13 (Full-text search)** - migration `20260806204719_add_recipe_search_vector` (generated weighted tsvector + GIN, applied to Neon; schema has `searchVector Unsupported("tsvector")?`). `src/lib/search.ts` (`toWebsearchQuery` + 4 tests, `searchRecipeIds`, `hydrateRecipeCards`, `searchRecipes`, `searchUsers`), `GET /api/search?q=&type=`. Explore: server passes `ftsResultIds` for `?q=`; client merges FTS rank order with instant substring matches and debounce-syncs typing to the URL (400 ms router.replace). NOTE: `prisma migrate dev` HANGS in non-interactive shells - use `--create-only` then `prisma migrate deploy`.
- ✅ **Task 14 (Command palette)** - `src/components/CommandPalette.tsx` mounted in Navbar (which also got a `Ctrl K` kbd hint). Ctrl/Cmd+K, debounced /api/search, arrow-key nav with wrap, quick links (incl. /shopping-list which ships in Task 20). All setState kept out of effect bodies (react-hooks/set-state-in-effect is enforced: resets live in the keydown handler, fetch state in the timer callback).
- ✅ **Task 15 (Similar recipes)** - `src/lib/similar.ts` (single raw SQL: shared ingredients x2 + shared tags, excludes self/direct forks/private, cached 1 h), `SimilarRecipes` strip under recipe page tabs.
- ✅ **Task 16 (OG images)** - `opengraph-image.tsx` for recipe + profile (nodejs runtime for Prisma; dark card, yellow accent bar, avatar, stats; fallback brand card for unknown slugs).
- ✅ **Task 17 (JSON-LD + metadata)** - `src/lib/jsonld.ts` (`recipeJsonLd`, `profileJsonLd`, `jsonLdScript` escapes `<`). Recipe + profile pages have `generateMetadata` and inline JSON-LD scripts; root layout got `metadataBase` (`NEXT_PUBLIC_APP_URL`), title template, OG/twitter defaults.
- ✅ **Task 18 (sitemap/robots/RSS)** - `src/app/sitemap.ts` (force-dynamic - MUST stay so CI builds don't hit the DB), `robots.ts` (disallows /api, /settings, /onboarding, /shopping-list), `/[username]/rss.xml` route (RSS 2.0, 1 h cache header; profile metadata advertises it via alternates). All smoke-tested 200.
- ✅ **Task 19 (Scaling + units)** - `src/lib/units.ts` (`Amount`, `scaleAmount` with kitchen-fraction snapping, `formatAmount` with vulgar fractions, `convertUnit` picking readable units, `CONVERTIBLE_UNITS`; 17 tests). New `IngredientsPanel` (client) on the recipe tab: servings stepper, As written/Metric/Imperial toggle, grouped by component. DEVIATION: cook mode did NOT get the panel (its API shape lacked amounts; the API now returns a flat `ingredients` array so a successor could add it).
- ✅ **Task 20 (Shopping list)** - `src/lib/shopping-list.ts` (`mergeIngredients` - base-unit summing, same-unit unknown merge, incompatible split; 7 tests; localStorage keys `forkable:shopping-list:v1` + `forkable:shopping-checked:v1`). Add/remove button lives in the IngredientsPanel header (captures current scale factor). `/shopping-list` page (client, works logged-out): source-recipe chips, checkboxes persisted, Copy as text, Print (print CSS hides nav), Uncheck all. Navbar cart icon with live count badge. `GET /api/recipes/[u]/[s]` now also returns flat `ingredients[]`.
- ✅ **Task 21 (Ingredient pages)** - `/ingredients` (searchable paginated grid, 48/page, usage counts + macro chips) and `/ingredients/[slug]` (macro panel with source badge, `PantryToggleButton` client component, top-12 recipes via `hydrateRecipeCards`). Footer link added.
- ✅ **Task 22 (Views + insights)** - `RecipeDailyStat` model (migration `20260806211309`; WARNING: `prisma migrate dev --create-only` emits bogus DROP statements for the generated `searchVector` column - always strip everything except your intended DDL from generated migrations; if a deploy fails use `prisma migrate resolve --rolled-back <name>`; if the advisory lock sticks, kill the holder via `pg_terminate_backend` on pids in pg_locks WHERE locktype='advisory'). `POST /api/recipes/[u]/[s]/view` (rate-limited 3/hr/ip/recipe, always 204), `ViewTracker` (5 s dwell), `/[username]/[recipe]/insights` owner-only dashboard: stat tiles with 7d deltas + hand-rolled SVG area chart (yellow gradient, hover tooltip, aria-label). Also FIXED pre-existing bug: recipe Edit button was hardcoded to username "shrey" - now `isOwnRecipe`, and owners get an Insights button. NOTE: react-hooks/purity forbids `Date.now()` inside (server) component bodies - hoist date math into module-level helpers.
- ✅ **Task 23 (PWA)** - `src/app/manifest.ts`, `public/sw.js` (hand-rolled: cache-first for `/_next/static` + `/icons`, network-first-with-fallback for navigations, skips `/api/`), `ServiceWorkerRegistrar` (production-only registration, mounted in root layout), icons generated via `scripts/gen-icons.ts` (sharp, inline SVG fork mark) into `public/icons/`.
- ✅ **Task 24 (AI)** - `src/lib/ai.ts` (`aiEnabled()`, `suggestSubstitutions`, `enrichRecipe`; model `claude-haiku-4-5`, `@anthropic-ai/sdk`). `POST /api/ai/substitutions` (login-gated, 10/hr rate limit, 24h cache per recipe) and `POST /api/ai/enrich` (same gating, no cache - one-shot at recipe-creation time). `SubstitutionsCard` (click-to-load, hides itself permanently on a 503 so a deployment without the key never shows a dead button) mounted on the recipe page above Instructions. New-recipe wizard has a "Suggest with AI" button next to the Description field that fills description + tags from name/ingredients/steps.
- ✅ **Task 5 (Docker)** - `docker-compose.yml` (postgres:16-alpine, port 5433 to avoid local conflicts), `.env.docker.example`, `npm run db:local` script.
- ✅ **Task 6 (Sentry)** - `src/instrumentation.ts` (server init + `onRequestError` hook) and `src/instrumentation-client.ts`, both no-op without `SENTRY_DSN`/`NEXT_PUBLIC_SENTRY_DSN`. Confirmed `npm run build` passes with zero Sentry env vars set. SETUP.html has Sentry section.
- ✅ **Task 25 (README + em-dash audit + CLAUDE.md sync)** - README.md fully rewritten (architecture diagram, feature tour, all 21 models, env var table, testing section). Em-dash audit: 3 real UI-facing strings fixed (feed empty state, taste-test-count separator, comment placeholder); all remaining `—` occurrences are in code comments, which the project rule allows. CLAUDE.md Current Sprint + Backlog sections updated to reflect the plan is complete.

**ALL 25 TASKS IN THIS PLAN ARE COMPLETE.** Final verification before the closing push: `npm run lint` (0 problems), `npx tsc --noEmit` (clean), `npm test` (83/83 passing), `npm run build` (all routes compile, including OG images, sitemap, AI routes, and Sentry instrumentation - all with zero optional env vars set).
