...
export function OnboardingClient() {
  ...
  const handleThemeChange = (t: "dark" | "light" | "system") => {
    setThemeState(t);
    // Our theme provider only supports dark/light; map system → dark (default)
  ...
  const handleFinish = async () => {
    ...
    try {
      ...
      // Refresh the JWT so the session reflects the new username immediately
      await refreshSession();
    } catch {
      // if save fails, still proceed
    }
    setSaving(false);
    // Redirect to their profile using the new username they chose (or keep exploring)
    const dest = username ? `/${username}` : "/explore";
    router.push(dest);
  };
